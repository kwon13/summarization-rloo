...
class Args:
    ...
    total_episodes: Optional[int] = 2500
    # total_episodes: Optional[int] = 8
    ...
    query_dataset: str = "fiveflow/summary-rlhf-filtered"
    """the query dataset"""
    reward_model_path: str = ""
    """the path to the reward model"""
    sft_model_path: str = "EleutherAI/pythia-160m"
    """the path to the sft model"""
    label_dataset: str = "fiveflow/summary-rlhf-reward"
...
class ScalarModelConfig(PretrainedConfig):
    def __init__(
        self,
        base_model: str = "maywell/EXAONE-3.0-7.8B-Instruct-Llamafied",
        base_config: PretrainedConfig = AutoConfig.from_pretrained("maywell/EXAONE-3.0-7.8B-Instruct-Llamafied"),
        hidden_size: int = 768,
        bias: float = 0.0,
        **kwargs,
    ):
...
class ScalarModel(PreTrainedModel):
    ...
    def __init__(self, config: ScalarModelConfig):
        ...
        self.scalar_head = layer_init(
            nn.Linear(self.config.hidden_size, 1).to(dtype=torch.bfloat16),
...
if __name__ == "__main__":
    ...
    # load dataset
    dataset = load_dataset(args.label_dataset, split="train").shuffle()
    ...
    norm_dataset = load_dataset(args.query_dataset, split="train").select(range(1000))
